...
export async function GET(req: NextRequest) {
  ...
  const { data: log } = await supabase
    .from("sync_logs")
    .insert({ sync_type: "scores", status: "running" })
    .select("id")
    .single();
  ...
  try {
    ...
    const { processed, pointsCalculated } = await syncScores(competitionId);
    ...
    return NextResponse.json({ success: true, processed, pointsCalculated });
  } catch (err) {
